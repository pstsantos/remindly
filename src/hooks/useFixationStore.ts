...
export function useFixationStore() {
  ...
  const bookAllFutureOccurrences = useCallback((
    ...
    // If heavy fixation, first revisit is in 1 day, then resume normal ladder
    const startFrom = currentSuccessCount + 1;
    const maxCount = Math.max(practiceSetCount, 6) + 1; // go a bit beyond set count for maintenance
